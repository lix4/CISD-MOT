...
def match_preds_with_gt(pred_boxes, gt_boxes, gt_labels, scores=None):
    """
    pred_boxes: (Mp,4) torch/np, xyxy, 224×224 坐标
    gt_boxes  : (Mg,4)
    gt_labels : (Mg,)
    scores    : (Mp,) 可选，若给则按分数降序配对
    返回: list[int]，与 pred_boxes 对齐的标签，未匹配为 -1
    """
    ...
    for pi in order:
        ...
        for gi in range(Mg):
            if gt_used[gi]:
                continue
            # print("A", pred_boxes[pi], gt_boxes[gi])
            iou = box_iou_xyxy(pred_boxes[pi], gt_boxes[gi])
            if iou > best_iou:
                best_iou, best_g = iou, gi
        if best_g != -1:
            assigned[pi] = int(gt_labels[best_g])  # 0/1/2
            gt_used[best_g] = True
    return assigned
...
def generate_tracks_json(dataset: listDataset, kwargs, gt=True, save_path='output_tracks_stream.json'):
    dataset.train = False
    ...
    with open(save_path, 'w') as f:
        ...
        for idx, item in enumerate(tqdm(loader, desc="Extracting tracks", disable=True)):
            ...
            video_id = sample['video_id'][0] if isinstance(sample['video_id'], list) else sample['video_id']
            print("########## ", video_id, " ##########")
            ...
            if not gt:
                ...
                with torch.no_grad():
                    ...
                    ##### lightwight iou tracker #####
                    # tracks = []
                    # link_thr = 0.5
                    # for t in range(16):
                    #     bxs  = frame_boxes[t]; lbls = frame_labels[t]
                    #     assigned = [False]*len(bxs)

                    #     # ① 先接到已有轨迹
                    #     for trk in tracks:
                    #         prev_box = trk['boxes'][-1]
                    #         best_iou, best_idx = 0., -1
                    #         if prev_box is not None:
                    #             for i, bx in enumerate(bxs):
                    #                 if assigned[i]: continue
                    #                 iou = box_iou_xyxy(prev_box, bx)
                    #                 if iou > best_iou:
                    #                     best_iou, best_idx = iou, i
                    #         if best_iou > link_thr:
                    #             trk['boxes'] .append(bxs[best_idx].tolist())
                    #             trk['labels'].append(lbls[best_idx])
                    #             assigned[best_idx] = True
                    #         else:
                    #             trk['boxes'] .append(None)
                    #             trk['labels'].append(-1)
                    ...
                    #         if not assigned[i]:
                    #             tracks.append({
                    #                 'boxes' : [None]*t + [bx.tolist()],
                    #                 'labels': [-1]*t   + [lbls[i]]
                    #             })

                    # # ----------- (C) 补齐 & 转 list -----------
                    # for trk in tracks:
                    #     pad = 16 - len(trk['boxes'])
                    #     if pad > 0:
                    #         trk['boxes'] .extend([None]*pad)
                    #         trk['labels'].extend([-1]*pad)

                    # # 变成 bboxes / labels / mask 三个 list，方便后面写 JSON
                    # N = len(tracks)
                    # bboxes, labels, mask = [], [], []
                    # for trk in tracks:
                    #     bboxes.append(trk['boxes'])
                    #     labels.append(trk['labels'])
                    #     mask.append([bx is not None for bx in trk['boxes']])
                    ##### lightwight iou tracker #####
...
if __name__ == '__main__':
    ...
    generate_tracks_json(dataset, kwargs, gt=False, save_path='valid_tracks_yolo.json')
