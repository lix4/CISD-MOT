...
def build_dataloaders(batch_size=8):
    ...
    val_ds = CustomYOLODataset(
        "./meta-files/testlist_e2e_new_1.txt",
...
if __name__ == "__main__":
    ...
    train_cmd.add_argument("--job-name",       type=str)
    ...
    if args.mode == "train":
        ...
        model     = load_model(nc=1, ckpt=args.weights, device=device)
        ...
        for epoch in range(args.epochs):
            ...
            # --- 验证 ---
            with torch.no_grad():
                map50, map5095 = evaluate_map(model.eval(), val_loader, device)
            print(f"[Val] Epoch {epoch+1:03d} | mAP@50={map50:.4f} | mAP@50-95={map5095:.4f}")
            exit()
            # 保存最新 & 最佳
            save_dir = os.path.join("runs", args.job-name)
    ...
    # ------------------------------------ test ------------------------------------
        ...
        model = load_model(nc=1, ckpt=args.weights, device=device).eval()
