...
from models.MOT_IVD_v1_7_ab_bydg import MOT_IVD_v1_7
import torch.nn.functional as F
from sklearn.metrics import f1_score, average_precision_score
from datasets.dataset_mot_debug import listDataset, avivd_collate_fn
import torchvision
import os
...
def validate_one_epoch(model, dataloader, loss_n, device, num_classes=3):
    ...
    with torch.no_grad():
        for batch in dataloader:
            _, bboxes_gt, bboxes_yolo, mask, audio, labels, video_ids, _ = batch
            if bboxes_yolo.shape[0] == 0:
                continue
            # clips = clips.to(device)
            # clips = clips.permute(0, 2, 1, 3, 4)
            bboxes_gt = bboxes_gt.to(device)
            bboxes_yolo = bboxes_yolo.to(device)
            mask = mask.to(device)[:,-1:]
            audio = audio.to(device)
            labels = labels.to(device)[:,-1:]
            # print(clips.shape, audio.shape)
            logits, joint_emb_n = model(bboxes_gt, bboxes_yolo, audio)  # [BN, L, C]
            ...
            # ---------- ❶ 错分样本调试 ----------
            wrong_mask = (preds != labels.squeeze(1)) & mask.squeeze(1).bool()
            if wrong_mask.any():
                for idx in torch.nonzero(wrong_mask, as_tuple=False).squeeze(1).cpu():
                    vid       = video_ids[idx]
                    last_bbox = (bboxes_yolo[idx, -1] if bboxes_yolo.dim() == 3 else bboxes_yolo[idx]).cpu().tolist()
                    pred_lbl  = preds[idx].item()
                    gt_lbl    = labels[idx, 0].item()
                    print(f"[VAL-Debug] Mis-class | video_id: {vid} | last_bbox: {last_bbox} "
                        f"| pred: {pred_lbl} | gt: {gt_lbl}")
            # ---------- ❶ ----------
...
def train_one_step(model, batch, loss_n, optimizer, device):
    ...
    Returns:
        - loss value (float)
    """
    _, bboxes_gt, bboxes_yolo, mask, audio, labels, _, _ = batch  # [BN, L, 4], [BN, L], [BN, 128, 469], [BN, L]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train / Validate MOT_IVD_v1_7 classifier")
...
if __name__ == "__main__":
    args = parse_args()
    checkpoint_dir = 'MOT_classification_cl_v1_7_yolo_ab_bydg'
    use_cuda = True
    num_workers = 8
    batch_size = 16
    ...
    train_dataset = listDataset(
        ...
        json_path_gt='./datasets/train_tracks.json',
        json_path_yolo='./datasets/train_tracks_yolo_1_corrected.json',
        load_frames=False,
    ...
    valid_dataset = listDataset(
        ...
        json_path_gt='./datasets/valid_tracks.json',
        json_path_yolo='./datasets/valid_tracks_yolo_1_corrected.json',
        load_frames=False,
        transform=T.ToTensor()
    )
    ...
    # ---------- 只验证 ----------
    if args.val_only:
        f1, mAP = validate_one_epoch(model, valid_loader, sup_con_l, device)
