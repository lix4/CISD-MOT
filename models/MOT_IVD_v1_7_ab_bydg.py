...
class MOT_IVD_v1_7(nn.Module):
    ...
    def forward(self, bboxes_gt, bboxes_yolo, audio):
        BN, L, _ = bboxes_gt.shape  # L 应为 16
        if BN == 0:
            return torch.empty(0, self.num_classes, device=bboxes_gt.device)

        # ===== bbox delta =====
        delta = bboxes_yolo[:, 1:] - bboxes_yolo[:, :-1]
        ...
        bboxes_emb = self.bbox_mlp(bboxes_gt)  # [BN,16,d]
