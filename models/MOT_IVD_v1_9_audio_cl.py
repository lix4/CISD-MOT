...
class MOT_IVD_v1_9(nn.Module):
    ...
    def __init__(self, num_classes=3, d=256, heads=4):
        ...
        # bbox & delta branches
        self.delta_mlp = nn.Sequential(nn.Linear(2, 128), nn.ReLU(), nn.Linear(128, d))
        self.delta_conv = nn.Conv1d(d, d, 3, padding=1)

        self.bbox_mlp = nn.Sequential(nn.Linear(2, 128), nn.ReLU(), nn.Linear(128, d))
        ...
        cl_state_dict = torch.load("../CISD-Research/run/trainlist_audio_encoder_ESC50/checkpoint.pt")
        new_state_dict = OrderedDict()
        for key, value in cl_state_dict.items():
            if key.startswith('module.'):
                new_key = key[len('module.'):]
            else:
                new_key = key
            new_state_dict[new_key] = value
        cl_state_dict = new_state_dict
        self.mob.load_state_dict(cl_state_dict)
        # 去掉 mobilenet 的头
        ...
        self.mob.classifier = nn.Identity()
        self.mob.eval()  # 切换到eval模式
        for param in self.mob.parameters():
    ...
    def forward(self, bboxes, audio):
        ...
        if BN == 0:
            return torch.empty(0, self.num_classes, device=centers.device)
        ...
        centers_emb = self.bbox_mlp(centers)  # [BN,16,d]
        centers_emb = self.bbox_conv(centers_emb.permute(0, 2, 1)).permute(0, 2, 1)
        ...
        bx_proj = self.bbox_down(centers_emb).reshape(BN, -1)   # 2048
        ...
        query = self.fuse_proj(delta + centers_emb)
...
if __name__ == "__main__":
    ...
    # 创建模型实例
    model = MOT_IVD_v1_9(num_classes=C).cuda()
