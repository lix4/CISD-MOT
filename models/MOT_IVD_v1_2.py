...
class MOT_IVD_v1_2(nn.Module):
    def __init__(self, num_classes, embed_dim=256, num_heads=4, num_encoder_layers=12, num_decoder_layers=6, max_seq_len=16):
        ...
        # === Classification Head ===
        self.cls_head = nn.Linear(embed_dim, self.num_classes)
    ...
    def forward(self, bboxes, audio):
        ...
        # print(BN, L , delta.shape==bboxes.shape)
        # print(delta)

        # === Encode bbox
        bbox_feat = self.bbox_encoder(delta)  # [BN, L, D]
...
if __name__ == "__main__":
    ...
    # 创建模型实例
    model = MOT_IVD_v1_2(num_classes=C)
