...
# ---------- Main Model ----------
class MOT_IVD_v1_6(nn.Module):
    def __init__(self, num_classes=3, d=256, heads=4):
        super(MOT_IVD_v1_6, self).__init__()
        ...
        # downsample for cl project
        self.audio_down = nn.Sequential(
            # kh=4, kw=4, stride=(2,2), padding=1  ->  4×15 → 2×7
            nn.Conv2d(576*6, 128, kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True)
        )

        self.bbox_down = nn.Sequential(
            nn.Linear(256, 128),
            nn.ReLU(inplace=True)
        )

        self.cl_projector = nn.Linear(3840, 256)
    ...
    def forward(self,bboxes,audio):
        ...
        # === audio patch ===
        B, _,T,F=audio.shape              # [BN,6,128,469]
        audio=audio.view(-1,1,T,F).repeat(1,3,1,1)           # 灰度→3ch
        feat=self.mob(audio)              # [BN*6,576,4,15]
        feat=feat.view(BN,-1,4,15)        # [BN,3456,4,15]

        proj_a = self.audio_down(feat)
        proj_a = proj_a.view(BN, -1)

        bx_proj = self.bbox_down(bx)
        bx_proj = bx_proj.view(BN, -1)
        # print(proj_a.shape, bx_proj.shape)

        joint_emb = torch.cat([bx_proj, proj_a], dim=1)
        # print(joint_emb.shape)
        joint_emb_n = torch_F.normalize(self.cl_projector(joint_emb), dim=1)
        # print(joint_emb_n.shape)

        # print(proj_a.shape, bx.shape)
        ...
        x = self.conv(fused)  # [B, D, T]
        # print(x.shape)
        x = x.squeeze(1)
        # print(x.shape)
        # x = x.mean(dim=2)  # [B, D, 1]
        # x = x.transpose(1, 2)  # [B, 1, D]

        logits=self.cls(x)                # [BN,16,C]
        # print(logits.shape, joint_emb_n.shape)
...
if __name__ == "__main__":
    ...
    # 创建模型实例
    model = MOT_IVD_v1_6(num_classes=C).cuda()
